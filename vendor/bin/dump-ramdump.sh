...
if [ -e /proc/last_debug_info ]; then
	dd if="/proc/last_debug_info" of=$FILE_PATH/$LASTDEBUGINFO bs=4m
else
	echo "/proc/last_debug_info does not exist!" >> $FILE_PATH/$LASTDEBUGINFO
fi
