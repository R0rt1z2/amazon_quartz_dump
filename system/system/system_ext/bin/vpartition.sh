...
then
    ...
    fbe=$(getprop "ro.crypto.type")
    log -p d -t $TAG "ro.crypto.state=$crypto_state, vold.decrypt=$vold_decrypt"
    if [ "$crypto_state" == "encrypted" ] && [ "$vold_decrypt" != "trigger_restart_framework" ] && [ "$fbe" != "file" ]
    then
        log -p i -t $TAG "device is encrypted, skip virtual partition FDE"
        exit 0
    fi
