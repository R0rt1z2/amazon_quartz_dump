...
MINERVA_WIFI_PREDEFINED="30pfp83p:xvb0/2/05330400:100:_softwareVersion=;SY,_buildType=;SY,\
_platform=;SY,_timeZone=;SY,_countryOfResidence=;SY,_otaGroupName=;SY,_deviceId=;SY,"
DELAY=120
...
function log_to_minerva
{
	local tmp_str=$1
	local minerva_str=$MINERVA_WIFI_PREDEFINED$tmp_str

	log -m -t "AmazonMinervaLoggerClient" "$minerva_str"
}
	...
	case $1 in
		chip_active)
			...
			if [ $# -eq 5 ]; then
				...
				log_to_minerva "operation=conn-soc-active;SY,sum=$3;FL,key=$2;SY,metadata=$4;SY,metadata1=$5;SY:"
			else
		...
		host_wakeup)
			...
			if [ $# -eq 3 ]; then
				...
				log_to_minerva "operation=wifi-num-wakeup-host;SY,sum=$3;FL,key=$2;SY:"
			else
				echo "Error in logging wifi-num-wakeup-host, requires 3 parameters, but only receives $# parameters"
			fi
			;;
		ant_switch)
			# ant switch triggered by screen rotation and implemented in wifi driver
			...
			if [ $# -eq 4 ]; then
				...
				log_to_minerva "operation=AntSwitch;SY,sum=$3;FL,key=$2;SY,metadata=$4;SY:"
			elif [ $# -eq 5 ]; then
				...
				log_to_minerva "operation=AntSwitch;SY,sum=$3;FL,key=$2;SY,metadata=$4;SY,metadata1=$5;SY:"
			else
				echo "Error in logging AntSwitch, requires 4 or 5 parameters, but only receives $# parameters"
			fi
			;;
		fw_ant_switch)
			# ant switch triggered by low rssi and implemented in wifi firmware
		...
		wifi_band)
			...
			if [ $# -eq 4 ]; then
				...
				log_to_minerva "operation=Band;SY,sum=1;FL,key=$2;SY,metadata=$3;SY,metadata1=$4;SY:"
			elif [ $# -eq 3 ]; then
				LOG_TO_KDM "wifiKDM:Band:fgtracking=false;DV;1,Counter=1;CT;1,unit=count;DV;1,metadata=!{\"d\"#{\"metadata\"#\"$3\"$\"key\"#\"$2\"}};DV;1:NR"
				log_to_minerva "operation=Band;SY,sum=1;FL,key=$2;SY,metadata=$3;SY:"
			else
